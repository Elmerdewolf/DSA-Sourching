...
async function renderDash() {
  ...
  const now = new Date();
  document.getElementById('today-str').textContent = `${now.getFullYear()}年${now.getMonth() + 1}月${now.getDate()}日 ${['周日', '周一', '周二', '周三', '周四', '周五', '周六'][now.getDay()]}`;
  ...
  pc.innerHTML = rp.length ? `<table style="width:100%;font-size:12.5px;"><thead><tr><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">产品</th><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">状态</th><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">日期</th></tr></thead><tbody>${rp.map(p => `<tr style="border-bottom:1px solid var(--gray-100);"><td style="padding:8px;">${p.name}</td><td style="padding:8px;">${stBadge(p.status)}</td><td style="padding:8px;color:var(--gray-500);">${fmtD(p.createdAt)}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state" style="padding:24px;"><p>暂无产品</p></div>';

  const rq = [...quotes].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);
  const qc = document.getElementById('dash-quotes');
  qc.innerHTML = rq.length ? `<table style="width:100%;font-size:12.5px;"><thead><tr><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">产品</th><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">物流商</th><th style="padding:8px;background:var(--gray-50);border-bottom:1px solid var(--gray-200);color:var(--gray-600);font-size:11px;">报价</th></tr></thead><tbody>${rq.map(q => `<tr style="border-bottom:1px solid var(--gray-100);"><td style="padding:8px;">${q.productName}</td><td style="padding:8px;">${q.logisticsName}</td><td style="padding:8px;font-weight:600;color:var(--primary);">$${q.avgPriceUSD}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state" style="padding:24px;"><p>暂无报价</p></div>';
}
function stBadge(s) {
  const m = { pending: '<span class="badge badge-yellow">待询价</span>', inquired: '<span class="badge badge-blue">已询价</span>', quoted: '<span class="badge badge-green">已报价</span>' };
...
async function fillProductUserSelects(defaultQuoteName = '', defaultInquiryName = '') {
  ...
  quoteSel.innerHTML = '<option value="">-- 请选择报价员 --</option>' + quoteUsers.map(u => {
    const name = u.name || u.username;
    return `<option value="${name}">${name}</option>`;
  }).join('');

  inquirySel.innerHTML = '<option value="">-- 请选择询价员 --</option>' + inquiryUsers.map(u => {
    const name = u.name || u.username;
    return `<option value="${name}">${name}</option>`;
  }).join('');
...
async function initProductFilters() {
  ...
  quoteSel.innerHTML = '<option value="">全部报价员</option>' + quoteUsers.map(u => `<option value="${u}">${u}</option>`).join('');
  inquirySel.innerHTML = '<option value="">全部询价员</option>' + inquiryUsers.map(u => `<option value="${u}">${u}</option>`).join('');
}
...
async function renderProducts() {
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    const inquiryLabel = p.status === 'pending' ? '去询价' : '修改询价';
    return `<div class="prod-card" onclick="event.target.closest('.prod-card-checkbox') ? event.stopPropagation() : openInquiryModal('${idEsc(p.id)}')">
      ...
      ${img}
      <div class="prod-card-body"><div class="prod-card-name">${p.name}</div><div class="prod-card-meta">${p.sku || '无SKU'} · ${p.status === 'pending' ? '待询价' : p.status === 'inquired' ? '已询价' : '已报价'}</div><div class="prod-card-meta">日期: ${p.opDate || '--'}</div><div class="prod-card-meta">报价员: ${p.quoteUserName || '-'} · 询价员: ${p.inquiryUserName || '-'}</div></div>
      <div class="prod-card-footer"><button class="btn btn-sm btn-primary" onclick="event.stopPropagation();openInquiryModal('${idEsc(p.id)}')">${inquiryLabel}</button><button class="btn btn-sm btn-outline" onclick="event.stopPropagation();openInquiryHistory('${idEsc(p.id)}')">询价记录</button><button class="btn btn-sm btn-outline" onclick="event.stopPropagation();editProduct('${idEsc(p.id)}')">编辑</button><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delProduct('${idEsc(p.id)}')">删除</button></div>
    </div>`;
...
async function batchDelProducts() {
  ...
  if (!confirm(`确认删除选中的 ${ids.length} 个产品？`)) return;
...
async function openAddProduct() {
  document.getElementById('mo-prod-title').textContent = '添加产品';
...
async function saveProduct() {
  ...
  if (!quoteUserName) { toast('请选择报价员', 'err'); return; }
  ...
  if (id) {
    ...
    if (idx >= 0) { products[idx] = { ...products[idx], ...data, updatedAt: new Date().toISOString() }; toast('产品已更新'); }
  } else {
...
async function editProduct(id) {
  ...
  document.getElementById('mo-prod-title').textContent = '编辑产品';
...
async function delProduct(id) { if (!confirm('确认删除该产品？')) return; const products = await load('products'); const filteredProducts = products.filter(p => p.id !== id); await save('products', filteredProducts); renderProducts(); await renderDash(); toast('产品已删除'); }
...
async function saveProdImg() {
  ...
  const products = await load('products');
  const idx = products.findIndex(p => p.id === id);
  if (idx < 0) { toast('产品不存在', 'err'); return; }
  const imgPreview = document.getElementById('prod-img-preview');
...
async function exportProducts() {
  const products = await load('products');
  if (products.length === 0) { toast('没有产品可导出', 'err'); return; }
  ...
  if (filtered.length === 0) { toast('筛选后没有产品可导出', 'err'); return; }
...
function openBatchAddProduct() {
  ...
  if (preview) preview.innerHTML = '<div style="padding:20px;text-align:center;color:var(--gray-400);">请选择模板文件</div>';
...
async function parseBatchProductImportData(content) {
  ...
  if (lines.length < 2) { toast('文件内容为空', 'err'); return; }
...
function renderBatchProductImportPreview() {
  ...
  if (batchProductImportData.length === 0) {
    preview.innerHTML = '<div style="padding:20px;text-align:center;color:var(--danger);">未找到有效产品数据</div>';
    importBtn.disabled = true;
    return;
  }

  let html = '<table style="width:100%;font-size:12px;"><thead><tr><th>产品名称</th><th>产品链接</th><th>报价员</th></tr></thead><tbody>';
...
async function saveBatchProduct() {
  if (batchProductImportData.length === 0) { toast('请先上传模板并解析', 'err'); return; }
  const products = await load('products');
  let added = 0;
...
async function renderInquiry() {
  ...
  if (pendingProducts.length === 0) { list.innerHTML = '<div class="empty-state"><p>暂无待询价产品</p></div>'; return; }
  list.innerHTML = pendingProducts.map(p => {
    ...
    const linkHtml = link
      ? `<div style="margin-top:6px;">
          <div style="font-size:11px;color:var(--gray-500);margin-bottom:4px;">产品参考链接</div>
    ...
    return `<div style="display:flex;gap:16px;padding:16px;border-bottom:1px solid var(--gray-100);align-items:center;">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:12px;color:var(--gray-500);">${p.sku || '无SKU'} · ${p.category || '未分类'}</div>
        ${linkHtml}
      </div>
      <button class="btn btn-primary" onclick="openInquiryModal('${p.id}')">去询价</button>
    </div>`;
  }).join('');
}
async function openInquiryModal(id) {
  ...
  if (link) {
    ...
    linkDisplay.innerHTML = '<span style="color:var(--gray-400);font-size:12px;">暂无产品参考链接</span>';
  }
  document.getElementById('inq-weight').value = p.weight || '';
  document.getElementById('inq-cost').value = p.cost || '';
  document.getElementById('inq-link1').value = p.purchaseLink1 || '';
...
function renderMoreRealImages() {
  ...
  if (!Array.isArray(inqMoreRealImages) || inqMoreRealImages.length === 0) {
    wrap.innerHTML = '<div class="thumb-empty">暂无更多实拍图</div>';
    return;
  }
...
async function saveInquiry() {
  ...
  if (!weight || weight <= 0) { toast('请输入有效的产品重量', 'err'); return; }
  if (!cost || cost <= 0) { toast('请输入有效的产品成本', 'err'); return; }
  const products = await load('products');
  const idx = products.findIndex(p => p.id === id);
  if (idx >= 0) {
...
async function openInquiryHistory(pid) {
  ...
  if (title) title.textContent = `${p.name} · 询价修改记录`;
  let html = `<div style="margin-bottom:10px;font-size:12px;color:var(--gray-600);">当前：${fmtInquiryNum(p.weight)}kg / ¥${fmtInquiryNum(p.cost)}</div>`;
  if (history.length === 0) {
    html += '<div class="empty-state" style="padding:20px;"><p>暂无修改记录</p></div>';
  } else {
...
async function initQuoteGen() {
  ...
  lsel.innerHTML = '<option value="">-- 选择物流商 --</option>' + logistics.map(l => `<option value="${l.id}">${l.name}</option>`).join('');
...
async function renderProductSelect() {
  ...
  if (filtered.length === 0) { grid.innerHTML = '<div style="padding:20px;text-align:center;color:var(--gray-400);">无产品</div>'; document.getElementById('qg-prod-count').textContent = '0'; return; }
  grid.innerHTML = filtered.map(p => {
...
async function renderQuoteCountries() {
  ...
  if (!lid) {
    cgrid.innerHTML = '<div style="color:var(--gray-500);font-size:12px;">请先选择物流商</div>';
    return;
  }
  ...
  if (!allCountries.length) {
    cgrid.innerHTML = '<div style="color:var(--gray-500);font-size:12px;">暂无国家数据</div>';
    return;
  }
  ...
  if (filteredCountries.length === 0) {
    cgrid.innerHTML = '<div style="color:var(--gray-500);font-size:12px;">未找到匹配国家</div>';
    return;
  }
  ...
  cgrid.innerHTML = filteredCountries.map(c => {
    ...
    const hint = available ? '' : ' <span style="color:var(--gray-400);font-size:11px;">无运费规则</span>';
    const title = available ? '' : '无运费规则';
...
async function generateQuote() {
  ...
  if (pids.length === 0) { toast('请至少选择一个产品', 'err'); return; }
  if (!lid) { toast('请选择物流商', 'err'); return; }
  if (qgSelectedCountries.length === 0) { toast('请至少选择一个国家', 'err'); return; }
  ...
  if (hasInvalid) { toast('有产品尚未完成询价', 'err'); return; }
  if (!logistics) { toast('物流商不存在', 'err'); return; }
  ...
  products.forEach(product => {
    qgSelectedCountries.forEach(countryCode => {
      ...
      if (!fr) {
        missingCountries.push(`${product.name} x ${countryName}(无规则)`);
        return;
      }
      ...
      if (!zone) {
        missingCountries.push(`${product.name} x ${getCountryName(countryCode)}(无匹配重量区间)`);
        return;
      }
  ...
  if (missingCountries.length > 0) {
    toast('以下国家缺少运费规则: ' + missingCountries.join(', '), 'err');
    return;
  }
  if (results.length === 0) {
    toast('所选国家暂无运费规则', 'err');
    return;
  }
  ...
  // 展示产品成本与运费成本（USD）
  let html = '<table class="quote-table"><thead><tr><th>产品</th><th>国家</th><th>产品成本(USD)</th><th>运费成本(USD)</th><th>报价(USD)</th></tr></thead><tbody>';
  results.forEach(r => {
...
function copyQuoteTable() {
  ...
  const text = '产品\t国家\t产品成本(USD)\t运费成本(USD)\t报价(USD)\n' + rows.join('\n');
  navigator.clipboard.writeText(text).then(() => toast('表格已复制到剪贴板'));
}
...
async function saveQuote() {
  ...
  toast(`报价已保存（${products.length} 个产品 x ${currentQuoteResult.results.length / products.length} 个国家）`);
  await nav('quotes');
}
...
async function renderQuotes() {
  ...
  if (quotes.length === 0) { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:40px;color:var(--gray-500);">暂无报价记录</td></tr>'; return; }
  const idEsc = (id) => JSON.stringify(id).slice(1, -1);
  tbody.innerHTML = quotes.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(q => {
    ...
    <td rowspan="${span}"><button class="btn btn-sm btn-outline" onclick="viewQuote('${idEsc(q.id)}')">查看</button><button class="btn btn-sm btn-danger" onclick="delQuote('${idEsc(q.id)}')" style="margin-left:8px;">删除</button></td>`;
...
async function viewQuote(id) {
  ...
  if (q.productImage || q.realImage) {
    ...
    if (q.productImage) imgHtml += `<div style="text-align:center;"><div style="font-size:11px;color:var(--gray-500);margin-bottom:4px;">产品图片</div><img src="${q.productImage}" style="width:100px;height:100px;object-fit:cover;border-radius:6px;border:1px solid var(--gray-200);"></div>`;
    if (q.realImage) imgHtml += `<div style="text-align:center;"><div style="font-size:11px;color:var(--gray-500);margin-bottom:4px;">实拍图</div><img src="${q.realImage}" style="width:100px;height:100px;object-fit:cover;border-radius:6px;border:1px solid var(--gray-200);"></div>`;
    imgHtml += '</div>';
  }
  let html = imgHtml;
  html += `<div style="margin-bottom:12px;font-size:13px;background:var(--gray-50);padding:10px;border-radius:6px;">
    <strong>产品:</strong> ${q.productName} &nbsp;|&nbsp; <strong>物流商:</strong> ${q.logisticsName} &nbsp;|&nbsp; <strong>利润率:</strong> ${q.profit}% &nbsp;|&nbsp; <strong>汇率:</strong> 1USD=¥${q.rate}
  </div>`;
  if (q.inquiryNote) html += `<div style="margin-bottom:12px;font-size:12px;color:var(--gray-600);background:#fffbeb;padding:8px;border-radius:6px;">备注: ${q.inquiryNote}</div>`;
  // 报价详情：包含成本（含利润）
  const detailRate = Number(q.rate);
  const detailProfitRate = 1 + Number(q.profit || 0) / 100;
  html += '<table class="quote-table"><thead><tr><th>产品</th><th>国家</th><th>产品成本(USD)</th><th>运费成本(USD)</th><th>报价(USD)</th></tr></thead><tbody>';
  q.details.forEach(r => {
    const cost = Number(r.cost);
    const freightCost = Number(r.freightCost);
...
async function delQuote(id) { if (!confirm('确认删除该报价？')) return; const quotes = await load('quotes'); const filteredQuotes = quotes.filter(q => q.id !== id); await save('quotes', filteredQuotes); await renderQuotes(); toast('报价已删除'); }
...
function updateQuoteExportBtn() {
  ...
  if (btn) { btn.disabled = checked === 0; btn.textContent = checked > 0 ? `批量导出 (${checked})` : '批量导出'; }
  if (delBtn) { delBtn.disabled = checked === 0; delBtn.textContent = checked > 0 ? `批量删除 (${checked})` : '批量删除'; }
  if (selAll) {
...
async function batchExportQuotes() {
  ...
  if (selected.length === 0) { toast('未找到选中的记录', 'err'); return; }
  ...
  link.click();
  toast(`已导出 ${selected.length} 条记录`);
}

async function batchDelQuotes() {
  const checked = document.querySelectorAll('.quote-chk:checked');
  if (checked.length === 0) return;
  if (!confirm(`确认删除选中的 ${checked.length} 条报价记录？`)) return;
  const quotes = await load('quotes');
...
function renderCountryList() {
  ...
  list.innerHTML = countries.map(c => `<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid var(--gray-100);align-items:center;gap:8px;">
    ...
    <div style="display:flex;gap:6px;">
      <button class="btn btn-sm btn-outline" onclick="editCountry('${c.code}')">编辑</button>
      <button class="btn btn-sm btn-danger" onclick="delCountry('${c.code}')">删除</button>
    </div>
  </div>`).join('');
}
...
function saveCountry() {
  ...
  if (!name || !code) { toast('请输入国家名称和缩写', 'err'); return; }
  if (!/^[A-Z]{2,3}(\/[A-Z]{2,3})?$/.test(code)) { toast('缩写应为 2-3 个大写字母，可用 / 分隔', 'err'); return; }
  const countries = loadCountries();
  const editCode = document.getElementById('country-edit-code').value;

  if (editCode) {
    const idx = countries.findIndex(c => c.code === editCode);
    if (idx < 0) { toast('未找到待编辑国家', 'err'); return; }
    if (code !== editCode && countries.some(c => c.code === code)) { toast('该缩写已存在', 'err'); return; }
  ...
  if (countries.some(c => c.code === code)) { toast('该缩写已存在', 'err'); return; }
...
async function exportQuotes() {
  const quotes = await load('quotes');
  if (quotes.length === 0) { toast('暂无数据可导出', 'err'); return; }
...
async function renderLogistics() {
  ...
  if (logistics.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:40px;color:var(--gray-500);">暂无物流商</td></tr>'; return; }
  const idEsc = (id) => JSON.stringify(id).slice(1, -1);
  tbody.innerHTML = logistics.map(l => `<tr>
    ...
    <td>${l.contact || '-'}</td>
    <td>
      <button class="btn btn-sm btn-outline" onclick="editLogi('${idEsc(l.id)}')">编辑</button>
      <button class="btn btn-sm btn-danger" onclick="delLogi('${idEsc(l.id)}')" style="margin-left:8px;">删除</button>
    </td>
  </tr>`).join('');
  refreshFreightLogi();
}
...
async function saveLogi() {
  const idEl = document.getElementById('el-id');
  const nameEl = document.getElementById('l-name');
  if (!nameEl) { toast('物流商表单未就绪，请刷新页面', 'err'); return; }
  const id = idEl ? idEl.value : '';
  const name = nameEl.value.trim();
  if (!name) { toast('请输入物流商名称', 'err'); return; }
  const logistics = await load('logistics');
  const codeEl = document.getElementById('l-code');
  const contactEl = document.getElementById('l-contact');
  ...
  if (id) {
    ...
    if (idx >= 0) { logistics[idx] = { ...logistics[idx], ...data2, updatedAt: new Date().toISOString() }; toast('物流商已更新'); }
  } else {
...
async function delLogi(id) { if (!confirm('确认删除该物流商？相关运费规则也会删除。')) return; const logistics = await load('logistics'); const filteredLogistics = logistics.filter(l => l.id !== id); await save('logistics', filteredLogistics); const freight = await load('freight'); const filteredFreight = freight.filter(f => f.logisticsId !== id); await save('freight', filteredFreight); await renderLogistics(); await renderFreight(); await renderDash(); toast('物流商已删除'); }
...
async function renderFreight() {
  ...
  let rows = [];
  filtered.forEach(f => {
    const logiName = logMap[f.logisticsId] || '未知';
  ...
  list.innerHTML = rows.map(r => `<tr>
    ...
    <td>
      <button class="btn btn-sm btn-outline" onclick="editFreight('${fidEsc(r.freightId)}')">编辑</button>
      <button class="btn btn-sm btn-danger" onclick="delFreightZone('${fidEsc(r.freightId)}',${r.zoneIndex})" style="margin-left:8px;">删除</button>
    </td>
  </tr>`).join('');
...
async function refreshFreightLogi() {
  ...
  const cur = sel.value;
  sel.innerHTML = '<option value="">全部物流商</option>' + logistics.map(l => `<option value="${l.id}">${l.name}</option>`).join('');
  sel.value = cur;
  const fsel = document.getElementById('f-logi');
  if (fsel) { const fcur = fsel.value; fsel.innerHTML = '<option value="">-- 选择物流商 --</option>' + logistics.map(l => `<option value="${l.id}">${l.name}</option>`).join(''); fsel.value = fcur; }
...
function openAddFreight() {
  document.getElementById('mo-freight-title').textContent = '添加运费规则';
...
function renderZones() {
  ...
  if (freightZones.length === 0) { wrap.innerHTML = '<div style="font-size:12.5px;color:var(--gray-400);">暂无区间，请添加</div>'; return; }
  wrap.innerHTML = freightZones.map((z, i) => `<div class="zone-row">
    <div class="zone-header"><span>区间 ${i + 1}</span><button class="btn btn-sm btn-danger" onclick="delZone(${i})">删除</button></div>
...
async function saveFreight() {
  ...
  if (!logisticsId) { toast('请选择物流商', 'err'); return; }
  if (!country) { toast('请输入国家', 'err'); return; }
  if (freightZones.length === 0) { toast('请至少添加一个重量区间', 'err'); return; }
  const countries = loadCountries();
  ...
  if (id) {
    ...
    if (idx >= 0) { freight[idx] = { ...freight[idx], ...data, updatedAt: new Date().toISOString() }; toast('运费规则已更新'); }
  } else {
...
async function editFreight(id) {
  ...
  document.getElementById('mo-freight-title').textContent = '编辑运费规则';
...
async function delFreight(id) { if (!confirm('确认删除该运费规则？')) return; const freight = await load('freight'); const filteredFreight = freight.filter(f => f.id !== id); await save('freight', filteredFreight); await renderFreight(); }
...
async function batchDelFreight() {
  ...
  if (checked.length === 0) return;
  if (!confirm(`确认删除选中的 ${checked.length} 个重量区间？`)) return;
  const freight = await load('freight');
...
function renderImportPreview() {
  ...
  if (importData.length === 0) { preview.innerHTML = '<div style="padding:20px;text-align:center;color:var(--danger);">未找到有效数据</div>'; document.getElementById('btn-confirm-import').disabled = true; return; }
  let html = '<table style="width:100%;font-size:12px;"><thead><tr><th>物流商</th><th>国家</th><th>重量段</th><th>单价</th><th>挂号费</th></tr></thead><tbody>';
...
async function confirmImport() {
  ...
  if (mode === 'replace') { if (!confirm('确定替换所有数据？')) return; }
  const logistics = await load('logistics');
...
async function initData() {
  const DATA_VERSION = 'v4.0';
  const storedVersion = localStorage.getItem('qms_version');

  // 检查文件系统支持
  if (!('showDirectoryPicker' in window)) {
    toast('您的浏览器不支持文件系统API，将使用 localStorage 存储', 'info');
  ...
  // 迁移旧数据提示
  if (storedVersion && storedVersion < 'v4.0') {
    toast('检测到旧版本数据，请点击"切换数据目录"来迁移到文件存储', 'info');
  }
  ...
  if (!DATA_DIR_HANDLE) {
    ...
    if (logistics.length === 0 && products.length === 0) {
      initLocalStorageData();
      toast('欢迎使用报价管理系统！请点击左侧"切换数据目录"选择数据文件夹', 'info');
    }
  }
}
...
async function chooseDataDirectory() {
  try {
    ...
    toast('数据目录已设置: ' + DATA_DIR_HANDLE.name);
    ...
    if (e.name === 'AbortError') {
      ...
      toast('未选择数据目录，将使用 localStorage', 'info');
    } else {
      console.error('选择目录失败:', e);
      toast('选择数据目录失败: ' + e.message, 'err');
    }
  }
}

async function fixedDataDirectory() {
  if (!('showDirectoryPicker' in window)) {
    toast('您的浏览器不支持文件系统API，无法固定目录', 'err');
    return;
  }
  const ok = confirm('将固定数据目录到项目的 data 文件夹。\n\n请在弹窗中选择“quote-system”项目根目录（或直接选择 data 文件夹）。');
  if (!ok) return;
  try {
    const rootHandle = await window.showDirectoryPicker({ mode: 'readwrite', startIn: 'documents' });
    let targetHandle = rootHandle;
    let displayName = rootHandle?.name || '';
    if (rootHandle?.name !== 'data') {
      targetHandle = await rootHandle.getDirectoryHandle('data', { create: true });
      displayName = displayName ? `${displayName}/data` : 'data';
    }
    DATA_DIR_HANDLE = targetHandle;
    DATA_DIR_NAME = displayName;
    await storeDirHandle(DATA_DIR_HANDLE);
    renderDataDirDisplay();
    toast('数据目录已固定到: ' + (displayName || DATA_DIR_HANDLE.name));
    ...
    if (e.name === 'AbortError') {
      ...
      console.error('固定目录失败:', e);
      toast('固定目录失败: ' + e.message, 'err');
    }
  }
}
...
function showStorageHint() {
  try {
    ...
    const host = location.hostname || location.host || '当前地址';
    toast(`当前使用本地存储（${host}）。刷新或更换访问地址（如 localhost/127.0.0.1）可能导致数据不可见，建议点击“切换数据目录”选择数据文件夹。`, 'info');
  } catch (e) {
    // ignore
  }
}
...
async function changeDataDirectory() {
  if (!('showDirectoryPicker' in window)) {
    toast('您的浏览器不支持文件系统API，无法切换目录', 'err');
    return;
  }
...
async function renderUsers() {
  ...
  if (!isAdmin()) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--gray-500);">仅管理员可管理账号</td></tr>';
    if (tip) tip.textContent = '当前账号没有账号管理权限';
    return;
  }
  ...
  if (users.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--gray-500);">暂无账号</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = users.map(u => `<tr>
    ...
    <td>${CURRENT_USER && CURRENT_USER.id === u.id ? '<span class="badge badge-blue">当前账号</span>' : '<span class="badge badge-gray">可登录</span>'}</td>
    <td>
      <button class="btn btn-sm btn-outline" onclick="editUser('${idEsc(u.id)}')">编辑</button>
      <button class="btn btn-sm btn-danger" onclick="delUser('${idEsc(u.id)}')" style="margin-left:8px;">删除</button>
    </td>
  </tr>`).join('');
}

function openAddUser() {
  if (!isAdmin()) { toast('仅管理员可添加账号', 'err'); return; }
  document.getElementById('mo-user-title').textContent = '添加账号';
  document.getElementById('eu-id').value = '';
  document.getElementById('u-username').value = '';
  document.getElementById('u-name').value = '';
  document.getElementById('u-role').value = 'quote';
  document.getElementById('u-password').value = '';
  document.getElementById('u-password-tip').textContent = '请设置登录密码';
  openM('mo-user');
}

async function editUser(id) {
  if (!isAdmin()) { toast('仅管理员可编辑账号', 'err'); return; }
  const users = await ensureUsersSeeded();
  const user = users.find(x => x.id === id);
  if (!user) { toast('未找到该账号', 'err'); return; }
  document.getElementById('mo-user-title').textContent = '编辑账号';
  document.getElementById('eu-id').value = user.id;
  document.getElementById('u-username').value = user.username;
  document.getElementById('u-name').value = user.name || '';
  document.getElementById('u-role').value = user.role || 'quote';
  document.getElementById('u-password').value = '';
  document.getElementById('u-password-tip').textContent = '留空表示不修改密码';
  openM('mo-user');
}

async function saveUser() {
  if (!isAdmin()) { toast('仅管理员可保存账号', 'err'); return; }
  ...
  if (!/^[a-zA-Z0-9_-]{3,20}$/.test(username)) { toast('账号需为 3-20 位字母、数字、下划线或中横线', 'err'); return; }
  if (!name) { toast('请输入姓名/备注', 'err'); return; }
  if (!USER_ROLES[role]) { toast('请选择有效角色', 'err'); return; }
  if (!id && !password) { toast('新增账号时必须设置密码', 'err'); return; }
  if (password && password.length < 6) { toast('密码至少 6 位', 'err'); return; }

  const users = await ensureUsersSeeded();
  const duplicated = users.find(u => u.username === username && u.id !== id);
  if (duplicated) { toast('该账号已存在', 'err'); return; }

  if (id) {
    const idx = users.findIndex(u => u.id === id);
    if (idx < 0) { toast('未找到待编辑账号', 'err'); return; }
    const target = users[idx];
    if (target.role === 'admin' && role !== 'admin') {
      const adminCount = users.filter(u => u.role === 'admin').length;
      if (adminCount <= 1) { toast('系统至少需要保留一个管理员账号', 'err'); return; }
    }
    users[idx] = {
      ...target,
      username,
...
async function delUser(id) {
  if (!isAdmin()) { toast('仅管理员可删除账号', 'err'); return; }
  if (!confirm('确认删除该账号？')) return;

  const users = await ensureUsersSeeded();
  const user = users.find(u => u.id === id);
  if (!user) { toast('未找到该账号', 'err'); return; }
  if (CURRENT_USER && CURRENT_USER.id === id) { toast('不能删除当前登录账号', 'err'); return; }
  if (user.role === 'admin' && users.filter(u => u.role === 'admin').length <= 1) {
    toast('系统至少需要保留一个管理员账号', 'err');
    return;
  }
